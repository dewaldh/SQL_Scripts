USE [sagex3]
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_BALANCE_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
...
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_CASH_TRANSACTION_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [DEVTST4].[BB_CASH_TRANSACTION_VALIDATION] 
(	
	@YBATNBR VARCHAR(25)	
)
AS
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_BANK_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_PATH_FIND]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_PATH_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
	...
	BEGIN
		...
		SET @ELA_PATH = CONCAT(@ELA_PATH,',',@LINE_ELA)
		IF NOT EXISTS (SELECT 1 FROM YICELA WHERE YELASTR_0 = @ORIGLIG_ELA)
		BEGIN	
	...
	ELSE
		BEGIN
			...
			BEGIN
				INSERT INTO YICELA ([UPDTICK_0],[YELASTR_0],[YELAEND_0],[YELAPATH_0],[CREDATTIM_0],[UPDDATTIM_0],[CREUSR_0],[UPDUSR_0],[AUUID_0])
				VALUES (1,@ORIGLIG_ELA,'n/a',@LINE_ELA, GETDATE(), GETDATE(),'SQL','SQL',NEWID())
			END
			ELSE
			BEGIN
				UPDATE YICELA
				SET YELAPATH_0 = @LINE_ELA, YELAEND_0 = 'n/a'
				WHERE YELASTR_0 = @ORIGLIG_ELA
			END	
		END
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ENTRY_VALIDATIONS]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_IE_TRANSACTION_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 -- EXEC [BB_SITE_VALIDATION] 'BB181100114'
 -- 
ALTER PROCEDURE [DEVTST4].[BB_IE_TRANSACTION_VALIDATION] --'BB181100114'
(	
	@YBATNBR VARCHAR(25)	
)
AS
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF @ELACOUNT = 1 AND @FCYCOUNT = 2
			BEGIN
				SELECT @ELALIG = TB.DYELA
				FROM 
				(
				  SELECT distinct(YELA_0) as DYELA
				  FROM YBBDETAILTEM ITM 
				  WHERE ITM.YTRANSNBR_0 = @MINTRANSNBR AND ITM.YBATNBR_0 = @YBATNBR AND ITM.YBATTYP_0 = @BATTYP
				) AS TB;

				IF @ELALIG is null or @ELALIG = ''
				BEGIN
					UPDATE YBBDETAILTEM
					SET YRESVAL_0 = 'NOELA'
					WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR
			...
			ELSE
			BEGIN
				SELECT @ELAAGR1 = YELA_0, @ELAFCY1 = YFCY_0 FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR
				AND YFCY_0 NOT IN (SELECT TOP 1 YFCY_0 FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR)
				SELECT @ELAAGR2 = YELA_0, @ELAFCY2 = YFCY_0 FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR
				AND YFCY_0 <> @ELAFCY1
				...
				BEGIN
					...
					-- Get ELA Paths to expand FCY1
					--Print '1'					
					EXEC [BB_ELA_PATH_FIND] @ELAAGR1,@ELAAGR2, @ELAPATH = @ELASHTPTH OUTPUT
					--Print '2'					
					IF (@ELASHTPTH <> '')
					BEGIN			
						Print @ELASHTPTH
					...
					BEGIN
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'FCY1ELAPATH'
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY1
						-- Update new path
						UPDATE YICELA
						SET YELAPATH_0 = @ELASHTPTH
						WHERE YELASTR_0 = @ELAAGR1
					END						
					-- Get ELA Paths to expand FCY2
					SET @ELASHTPTH = ''
					EXEC [BB_ELA_PATH_FIND] @ELAAGR2,@ELAAGR1, @ELAPATH = @ELASHTPTH OUTPUT

					IF (@ELASHTPTH <> '')
					BEGIN			
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = @ELAAGR2
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY2
						-- Update new path
						UPDATE YICELA
						SET YELAPATH_0 = @ELASHTPTH
						WHERE YELASTR_0 = @ELAAGR2
					END
					ELSE
					BEGIN
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'FCY2ELAPATH'
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY2
					END				
				END
				ELSE
				BEGIN
					-- ELA Provider site not found
					UPDATE YBBDETAILTEM
					SET YRESVAL_0 = 'ELAPROFCY'
					WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY1
					UPDATE YBBDETAILTEM
					SET YRESVAL_0 = 'ELAPROFCY'
					WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY2
			...
			END
		END
		-- Set Transaction number to next
		set @MINTRANSNBR += 1
	END

END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_SITE_VALIDATION]    Script Date: 6/12/2018 5:34:43 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 -- EXEC [BB_SITE_VALIDATION] 'BB20181000027'
 -- 
