USE [sagex3]
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_BALANCE_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
-- EXEC [BB_BALANCE_VALIDATION] 'BB20181100016'

ALTER PROCEDURE [DEVTST4].[BB_BALANCE_VALIDATION] 
(	
	@YBATNBR VARCHAR(25)
)
AS
...
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_CASH_TRANSACTION_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [DEVTST4].[BB_CASH_TRANSACTION_VALIDATION] -- 'BB19000205'
(	
	@YBATNBR VARCHAR(25)	
)
AS
BEGIN
	...
	BEGIN
		...
		IF @BATTYP = 2
		BEGIN -- 1
			--Updated the first time #DFCX1-344
			--Updated 27/02/2019 as per change request from Mark
			--Updated again 28/02/2019 as per change request - no to remove the validations for bank transfer again.
			--Cash Transfer : distinct bank accounts = 2 | lines = 2
			--Cash Transaction: : distinct bank accounts = 1 | lines >= 2
			...
			SELECT @MBNKCOUNT = COUNT(YBAN_0) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YBATTYP_0 = @BATTYP AND YBAN_0 <> ''	 
			SELECT @LIGCOUNT = COUNT(*) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YBATTYP_0 = @BATTYP
			SELECT @FCYCOUNT = COUNT(DISTINCT(YFCY_0)) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YBATTYP_0 = @BATTYP

			IF @FCYCOUNT = 1 --AND @BANKCOUNT = 2 AND @LIGCOUNT = 2
			BEGIN
				IF (@LIGCOUNT = 2 and @BANKCOUNT = 2) or (@LIGCOUNT >= 2  and @MBNKCOUNT = 1)
				BEGIN
					SET @PREVAL = 'CHKPASS'
				END
				ELSE
				BEGIN
					IF @LIGCOUNT > 2  and @MBNKCOUNT >= 2
					BEGIN
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'BANEMP' --Invalid number of bank accounts
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR

					END
					IF @LIGCOUNT = 2 and @BANKCOUNT < 2
					BEGIN
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'TRANLIG' --INVALID TRANSACTION FOR BANK TRANSFER
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR 
					END
				END
			END			
			...
			BEGIN
				UPDATE YBBDETAILTEM
				SET YRESVAL_0 = 'FCYREQ' --CANNOT HAVE MORE THAN 1 DISTINCT SITE
				WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR 
			END
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_BANK_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [DEVTST4].[BB_ELA_BANK_VALIDATION] 
(	
	@BANLIG VARCHAR(10),
	@YELALIG VARCHAR(50)
)
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_PATH_FIND]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 ...
 -- 
ALTER PROCEDURE [DEVTST4].[BB_ELA_PATH_FIND] 
(	
	@ELA1 VARCHAR(50),
	@ELA2 VARCHAR(50),
...
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ELA_PATH_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 ...
 -- 
ALTER PROCEDURE [DEVTST4].[BB_ELA_PATH_VALIDATION] --'MPL_DF_005_063_INT_2'
(	
	@LINE_ELA VARCHAR(50)
)
AS
...
END
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_ENTRY_VALIDATIONS]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 ...
 -- 
ALTER PROCEDURE [DEVTST4].[BB_ENTRY_VALIDATIONS] 
(	
	@YBATNBR VARCHAR(25)
)
AS
...
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_IE_TRANSACTION_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 -- EXEC [BB_IE_TRANSACTION_VALIDATION] 'BB181200049'
 -- 
ALTER PROCEDURE [DEVTST4].[BB_IE_TRANSACTION_VALIDATION] --'BB20181100085'
(	
	@YBATNBR VARCHAR(25)	
)
AS
BEGIN -- 1
	...
	DECLARE @BATTYP INT,
			...
			@FCY1    	 VARCHAR(10),
			@FCY2    	 VARCHAR(10),
			@SUMDRFCY1	 DECIMAL(9,2),
			@SUMCRFCY1   DECIMAL(9,2),
			@SUMDRFCY2	 DECIMAL(9,2),
			@SUMCRFCY2   DECIMAL(9,2)
	...
	SELECT @BATTYP = YBATTYP_0 from YBBDETAILTEM Where YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR	
		...
		BEGIN -- 3
			...
			SELECT @ELACOUNT = COUNT(DISTINCT(YELA_0)) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR and YELA_0 <> '' --Changed as per RRL 26-02
			IF (@ELACOUNT = 1 or @ELACOUNT = 2) AND @FCYCOUNT = 2
			BEGIN -- 4.1
				...
				-- CHECK SUM FOR DR/CR PER SITE NOT EQUAL TO 0
				SELECT DISTINCT top 1 @FCY1 = YFCY_0 FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR
				SELECT DISTINCT @FCY2 = YFCY_0 FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 <> @FCY1

				SELECT @SUMDRFCY1 = SUM(YDEB_0), @SUMCRFCY1 = SUM(YCDT_0) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @FCY1
				SELECT @SUMDRFCY2 = SUM(YDEB_0), @SUMCRFCY2 = SUM(YCDT_0) FROM YBBDETAILTEM WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @FCY2

				IF (@SUMDRFCY1 - @SUMCRFCY1 = 0)
				BEGIN -- 4.1.0
					UPDATE YBBDETAILTEM
					SET YRESVAL_0 = 'FCY1BAL0'
					WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @FCY1
				END
				IF (@SUMDRFCY2 - @SUMCRFCY2 = 0)
				BEGIN	
					UPDATE YBBDETAILTEM
					SET YRESVAL_0 = 'FCY2BAL0'
					WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @FCY2	
				END -- 4.1.0
				ELSE
				BEGIN -- 4.1.0				
					...
					IF (@ELAAGR1 <> '' or @ELAAGR2 <> '')
					BEGIN -- 4.1.1
						EXEC [DEVTST4].[BB_ELA_PATH_VALIDATION] @ELAAGR1
						EXEC [DEVTST4].[BB_ELA_PATH_VALIDATION] @ELAAGR2
					...
					BEGIN -- 4.1.2
						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'ELAGR1'
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY1

						UPDATE YBBDETAILTEM
						SET YRESVAL_0 = 'ELAGR2'
						WHERE YTRANSNBR_0 = @MINTRANSNBR AND YBATNBR_0 = @YBATNBR AND YFCY_0 = @ELAFCY2
					END -- 4.1.2*/
				END -- 4.1.0
			END -- 4.1
...
END -- 1
GO
/****** Object:  StoredProcedure [DEVTST4].[BB_SITE_VALIDATION]    Script Date: 29/04/2019 11:21:11 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
 ...
 -- 
ALTER PROCEDURE [DEVTST4].[BB_SITE_VALIDATION] 
(	
	@YBATNBR VARCHAR(25)
)
AS
